...
CREATE TABLE relatorio_unidade (
    id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    id_unidade INT(6) NOT NULL,
    valor_m3 INT(20) NOT NULL,
    valor_reais VARCHAR(20) NOT NULL,
...
CREATE TABLE relatorio_condominio (
    id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    id_condominio INT(6) NOT NULL,
    taxa_minima VARCHAR(20) NOT NULL, 
    taxa_m3_completa VARCHAR(20) NOT NULL, 
    taxa_m3 VARCHAR(20) NOT NULL, 
    consumo_condominio_m3 INT(6) NOT NULL, 
    mes INT(2) UNSIGNED ZEROFILL NOT NULL,
    ano INT(4) NOT NULL,
    CONSTRAINT consumo_mes_ano UNIQUE(id_condominio, mes, ano)
);
...
-- Trigger ao atualizar o Consumo de um Condomínio
DELIMITER $$
CREATE TRIGGER deleta_relatorio_consumo_condominio
    AFTER UPDATE
    ON consumo_condominio FOR EACH ROW
BEGIN
    DELETE FROM relatorio_unidade WHERE relatorio_unidade.id_unidade IN (
        SELECT unidade.id FROM unidade WHERE unidade.id_condominio = old.id_condominio
    ) AND relatorio_unidade.mes = old.mes AND relatorio_unidade.ano = old.ano;
    DELETE FROM relatorio_condominio WHERE relatorio_condominio.id_condominio = old.id_condominio
    AND relatorio_condominio.mes = old.mes AND relatorio_condominio.ano = old.ano;
END$$    
DELIMITER ;

-- Trigger ao atualizar o Consumo de uma Unidade
DELIMITER $$
CREATE TRIGGER deleta_relatorio_consumo_unidade
    AFTER UPDATE
    ON consumo_unidade FOR EACH ROW
BEGIN
    DELETE FROM relatorio_unidade WHERE relatorio_unidade.id_unidade IN (
        SELECT unidade.id FROM unidade 
        WHERE unidade.id_condominio = (SELECT unidade.id_condominio FROM unidade WHERE unidade.id = old.id_unidade)
    ) AND relatorio_unidade.mes = old.mes AND relatorio_unidade.ano = old.ano;
    DELETE FROM relatorio_condominio WHERE relatorio_condominio.id_condominio = (
        SELECT unidade.id_condominio FROM unidade WHERE unidade.id = old.id_unidade
    ) AND relatorio_condominio.mes = old.mes AND relatorio_condominio.ano = old.ano;
END$$    
DELIMITER ;
